...
import (
	...
	"github.com/renproject/multichain/chain/electra"
...
var _ = Describe("Electra", func() {
	Context("when submitting transactions", func() {
		Context("when sending Electra to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting ELECTRA_PK to the
				// value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("ELECTRA_PK")
				if pkEnv == "" {
					panic("ELECTRA_PK is undefined")
				}
				...
				client := bitcoincompat.NewClient(bitcoincompat.DefaultClientOptions().WithHost("http://127.0.0.1:15788"))
				...
				tx, err := electra.NewTxBuilder().BuildTx([]bitcoincompat.Output{output}, recipients)
				Expect(err).ToNot(HaveOccurred())
				...
				// Submit the transaction to the Electra node. Again, this
