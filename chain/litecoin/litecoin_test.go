...
var _ = Describe("Litecoin", func() {
	Context("when submitting transactions", func() {
		Context("when sending LTC to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting LITECOIN_PK to the
				// value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("LITECOIN_PK")
				if pkEnv == "" {
					panic("LITECOIN_PK is undefined")
				}
				wif, err := btcutil.DecodeWIF(pkEnv)
				Expect(err).ToNot(HaveOccurred())

				// PKH
				pkhAddr, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeCompressed()), &litecoin.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				pkhAddrUncompressed, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeUncompressed()), &litecoin.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				...
				client := litecoin.NewClient(litecoin.DefaultClientOptions().WithHost("http://127.0.0.1:19443"))
				...
				tx, err := litecoin.NewTxBuilder(&litecoin.RegressionNetParams).BuildTx(inputs, recipients)
				Expect(err).ToNot(HaveOccurred())
