...
var _ = Describe("NIX", func() {
	Context("when submitting transactions", func() {
		Context("when sending NIX to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting NIX_PK to the
				// value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("NIX_PK")
				if pkEnv == "" {
					panic("NIX_PK is undefined")
				}
				...
				client := bitcoincompat.NewClient(bitcoincompat.DefaultClientOptions().WithHost("http://127.0.0.1:16216"))
				...
				tx, err := nix.NewTxBuilder().BuildTx([]bitcoincompat.Output{output}, recipients)
				Expect(err).ToNot(HaveOccurred())
				...
				// Submit the transaction to the NIX node. Again, this
